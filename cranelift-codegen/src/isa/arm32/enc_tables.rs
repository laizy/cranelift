...
// arm32 recipe predicates.

/// arm32 recipe predicate table.
///
...
// arm32 instruction predicates.

/// arm32 instruction predicate table.
///
...
/// arm32 level 1 hash table for the CPU mode A32.
///
...
pub static LEVEL1_A32: [Level1Entry<u16>; 2] = [
    Level1Entry { ty: ir::types::INVALID, log2len: 0, offset: !0 - 1, legalize: 0 }, // narrow_flags
    Level1Entry { ty: ir::types::INVALID, log2len: !0, offset: 0, legalize: 0 },
];

/// arm32 level 1 hash table for the CPU mode T32.
///
/// This hash table, keyed by instruction controlling type, contains all the level 2
/// hash-tables offsets for the given CPU mode, as well as a legalization identifier indicating
/// which legalization scheme to apply when the instruction doesn't have any valid encoding for
...
/// arm32 recipe names, using the same recipe index spaces as the one specified by the
/// corresponding binemit file.
static RECIPE_NAMES: [&str; 0] = [
];

/// arm32 recipe constraints list, using the same recipe index spaces as the one
...
/// arm32 recipe sizing descriptors, using the same recipe index spaces as the one
...
pub static LEGALIZE_ACTIONS: [isa::Legalize; 1] = [
